...
# 1) Give unphysical Ωi0, h -> ΩΛ0
# 2) Give   physical ωi0    -> ΩΛ0, h derived
...
function integrate_ϕ(θ::Parameters; aini=1e-10)
    ...
    x0 = log(aini)
    return solve(ODEProblem(y′, y0, (x0, 0.0)))
end
...
function shoot(ω=1e3, ωr0=5.5e-5*0.67^2, ωm0=0.317*0.67^2, ϕ0_target = 1.0; maxiters=100, verbose=false)
    println("Fixing ω = $ω, ωr0 = $ωr0, ωm0 = $ωm0;")
    println("Varying ωΛ0, ϕini until ϕ0 = 1 and Ω0 = 1:")

    @assert ϕ0_target > 0.0
    ϕini_lo = 0.0       # lower bound, guaranteed to have ϕ0 < ϕ0_target (since it gives ϕ0 = 0)
    ϕini_hi = ϕ0_target # upper bound, guaranteed to have ϕ0 > ϕ0_target (since dlogϕ/dloga > 0)
